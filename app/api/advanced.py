"""
Advanced features blueprint: RAG, sentiment, scoring, callbacks, transfers, analytics, auth, action log.
"""
import logging
...
def add_knowledge():
    ...
    doc_id = add_document(
        ...
        industry=data.get("industry", ""), file_path=data.get("file_path", ""),
    )
...
def add_callback():
    ...
    cid = schedule_callback(
        lead_id=data["lead_id"], scheduled_at=data["scheduled_at"],
        channel=data.get("channel", "voice"), notes=data.get("notes", ""),
    )
...
def add_transfer():
    ...
    tid = request_transfer(
        lead_id=data["lead_id"], reason=data.get("reason", ""),
...
@advanced_bp.route("/api/auth/login", methods=["POST"])
def login():
